...
async def get_tasks(
    ...
    assert_access: Callable = Depends(Permissions(ModelPermission.READ)),
) -> list[TaskResponse]:
    tasks = await task_controller.get_all_by_author_id(request.user.id)
...
async def get_task(
    ...
    task = await task_controller.get_by(field='_id', value=task_uuid)
